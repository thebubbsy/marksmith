...
public class OpenXmlSyntaxHighlighter
{
    ...
    private readonly System.Collections.Generic.Dictionary<string, ILanguage> _languageDict;

    public OpenXmlSyntaxHighlighter()
    {
        _languageDict = new System.Collections.Generic.Dictionary<string, ILanguage>(StringComparer.OrdinalIgnoreCase);
        foreach (var lang in ColorCode.Languages.All)
        {
            if (lang != null && !string.IsNullOrWhiteSpace(lang.Id))
            {
                _languageDict[lang.Id] = lang;
                if (!string.IsNullOrWhiteSpace(lang.Name))
                {
                    _languageDict[lang.Name] = lang;
                }
            }
        }
        ...
        _parser = new LanguageParser(
            ...
            new ColorCode.Common.LanguageRepository(_languageDict));
    }
    ...
    public IEnumerable<Run> GetHighlightedRuns(string sourceCode, string languageId, ThemeDefinition theme)
    {
        ...
        if (!_languageDict.TryGetValue(normalizedId, out language) || language == null)
        {
            language = ColorCode.Languages.FindById(normalizedId);
        }

        if (language == null)
        {
            var plainRun = new Run(new Text(sourceCode) { Space = DocumentFormat.OpenXml.SpaceProcessingModeValues.Preserve });
            var plainRPr = new DocumentFormat.OpenXml.Wordprocessing.RunProperties();
            plainRPr.Append(new DocumentFormat.OpenXml.Wordprocessing.RunFonts { Ascii = "Consolas", HighAnsi = "Consolas" });
            plainRPr.Append(new DocumentFormat.OpenXml.Wordprocessing.NoProof());
            plainRun.RunProperties = plainRPr;
            yield return plainRun;
            yield break;
        }
        ...
        for (int i = 0; i < sortedBoundaries.Count - 1; i++)
        {
            ...
            rPr.Append(new DocumentFormat.OpenXml.Wordprocessing.NoProof());
