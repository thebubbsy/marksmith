...
public static class AppServices
{
    ...
    // thread-safe: the first accessor wins and everyone else blocks on it.
    private static readonly Lazy<Services.MarkdownHtmlService> _markdownHtml = new();
    public static Services.MarkdownHtmlService MarkdownHtml => _markdownHtml.Value;

    private static readonly Lazy<Services.LlmSourceService> _llmSource = new();
    public static Services.LlmSourceService LlmSource => _llmSource.Value;

    private static readonly Lazy<Services.HistoryService> _history = new();
    public static Services.HistoryService History => _history.Value;

    private static readonly Lazy<Services.GovernanceService> _governance = new();
    public static Services.GovernanceService Governance => _governance.Value;

    private static readonly Lazy<Services.UpdateService> _updates = new();
    public static Services.UpdateService Updates => _updates.Value;

    private static readonly Lazy<Services.BatchConvertService> _batchConvert = new();
    public static Services.BatchConvertService BatchConvert => _batchConvert.Value;

    private static readonly Lazy<Services.CloudStorageService> _cloudStorage = new();
    public static Services.CloudStorageService CloudStorage => _cloudStorage.Value;

    private static readonly Lazy<Plugins.PluginManager> _plugins = new();
    public static Plugins.PluginManager Plugins => _plugins.Value;

    private static readonly Lazy<Services.ExportCoordinator> _exportCoordinator = new();
    public static Services.ExportCoordinator ExportCoordinator => _exportCoordinator.Value;

    private static readonly Lazy<Services.VersionHistoryService> _versionHistory = new();
    public static Services.VersionHistoryService VersionHistory => _versionHistory.Value;

    private static readonly Lazy<Services.GoogleAuthService> _googleAuth = new();
    public static Services.GoogleAuthService GoogleAuth => _googleAuth.Value;

    private static readonly Lazy<Services.GoogleDocsExportService> _googleDocs = new();
    ...
    private static readonly Lazy<ViewModels.MainViewModel> _viewModel = new();
