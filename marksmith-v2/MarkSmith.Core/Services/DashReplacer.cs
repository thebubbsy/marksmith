...
public static partial class DashReplacer
{
    ...
    // Group 1 = protected spans (HTML comments, HTML tags/attributes, CriticMarkup deletions/insertions/substitutions/highlights/comments, CLI flags, autolinks, inline code,
    ...
    [GeneratedRegex(@"(<!--[\s\S]*?-->|</?[a-zA-Z!?:][^>]*>|\{--[\s\S]*?--\}|\{~~[\s\S]*?~~\}|\{\+\+[\s\S]*?\+\+\}|\{==[\s\S]*?==\}|\{>>[\s\S]*?<<\}|(?<!\w)--[a-zA-Z0-9_-]+|`+[^`\n]*?`+|\$\$[\s\S]*?\$\$|\$(?!\s)[^\$\n]*?\S\$|\]\([^)]*\)|https?://[^\s<>""'\(\)]+|(?m)^\s*\|?[\s|:-]+\|?\s*$)|(?<!-)--(?!-)", RegexOptions.Compiled)]
