...
namespace MarkSmith.Core.Office
{
    ...
    /// render (PNG data-URI) displayed in the preview pane. When Looking Glass (portal) mode is
    /// on, the page includes the same portal overlay + JS API (__portalSetBlur / __portalSetShape
    /// / __portalSetReveal) so the app's existing blur/unblur (Ctrl+Alt+X) and aperture controls
    /// keep working on the Word-accurate view.
    /// </summary>
    public static class WordFidelityPage
    {
        public static string Build(string imageDataUri, bool lookingGlassMode, bool stale)
        {
            string mime = imageDataUri.StartsWith("data:image/gif", StringComparison.OrdinalIgnoreCase) ? "gif" : "png";
...
#canvas img{max-width:100%;max-height:92vh;box-shadow:0 4px 24px rgba(0,0,0,.45);background:#fff;border-radius:4px;}
...
");
            ...
            sb.Append("<div id=\"canvas\"><img src=\"").Append(imageDataUri)
              .Append("\" alt=\"Word render\" draggable=\"false\"/></div>");
            ...
            {
                sb.Append("<div class=\"badge\">Word-accurate render (").Append(mime).Append(")</div>");
            }
