...
class Program
{
    static void Main()
    {
        var text = File.ReadAllText(@"C:\Users\Tony\.gemini\antigravity\scratch\marksmith\scratch\MainWindow_backup.xaml");

        var automation = Regex.Match(text, @"<Expander x:Name=""AutomationExpander"".*?</Expander>", RegexOptions.Singleline).Value;
        var styleCard = Regex.Match(text, @"<Border x:Name=""StyleCard"".*?</Border>", RegexOptions.Singleline).Value;
        var historyList = Regex.Match(text, @"<ListView x:Name=""HistoryList"".*?</ListView>", RegexOptions.Singleline).Value;

        var header = text.Substring(0, text.IndexOf("<!-- Left-to-right pipeline:"));

        var newXaml = header + @"        <!-- 3-Pane Looking Glass UI -->
...
" + historyList.Replace("MaxHeight=\"420\"", "").Replace("Grid.Row=\"1\"", "Grid.Row=\"1\" VerticalAlignment=\"Stretch\"") + @"
            </Grid>
...
";
        ...
        File.WriteAllText(@"C:\Users\Tony\.gemini\antigravity\scratch\marksmith\MdToPdf\MainWindow.xaml", newXaml);
    }
}
