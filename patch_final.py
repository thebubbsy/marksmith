...
# CENTER PANE
paste_panel_match = re.search(r'(<!-- Paste mode -->.*?)</ScrollViewer>', source_xml, re.DOTALL)
paste_panel = paste_panel_match.group(1) if paste_panel_match else ""
paste_panel = paste_panel.replace('Grid.Row="1"', 'Grid.Row="1"')
paste_panel = paste_panel.replace('Visibility="Collapsed"', 'Visibility="Visible"')

# we need to close PastePanel correctly, it ends with </Grid>.
# the regex match ends at </ScrollViewer>, so let's use the explicit string:
paste_panel_match2 = re.search(r'(<!-- Paste mode -->.*?<!-- Automated ingest)', source_xml, re.DOTALL)
if paste_panel_match2:
    paste_panel = paste_panel_match2.group(1)
    # The actual end of paste panel is the </Grid> before <!-- Automated ingest
