...
namespace MdToPdf.Core.AdvancedFeatures
{
    ...
    // ───────────────────────────────────────────────────────────────────
    // 7. Tabs — validates tab title attributes.
    // ───────────────────────────────────────────────────────────────────
    public class TabsDetector : IFeatureDetector
    {
        ...
        public (bool IsValid, double Confidence, string[] Errors) Validate(string rawBlock)
        {
            ...
            // Must contain at least one :::tab child
            var tabLines = lines.Where(l => l.TrimStart().StartsWith(":::tab", StringComparison.OrdinalIgnoreCase)).ToList();
            if (tabLines.Count == 0)
                errors.Add("No :::tab children found inside :::tabs block");

            // Each :::tab should have a title attribute
            foreach (var tl in tabLines)
            {
                if (!tl.Contains("title=", StringComparison.OrdinalIgnoreCase))
                    errors.Add($"Tab missing title attribute: '{tl.Trim()}'");
            }
